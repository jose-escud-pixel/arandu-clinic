...
export const EmpresaProvider = ({ children }) => {
  ...
  const applyFromUser = useCallback((userWithEmpresa) => {
  	// 🔥 FIX: Si no tiene empresa pero tiene empresas[0], usar la primera
  	if (userWithEmpresa?.empresa) {
    		setAndApplyEmpresa(userWithEmpresa.empresa);
  	} else if (userWithEmpresa?.empresas?.length > 0) {
    		// Si es admin/doctor con una sola empresa, aplicarla
    		setAndApplyEmpresa(userWithEmpresa.empresas[0]);
  	} else {
